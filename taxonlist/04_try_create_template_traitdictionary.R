...
# ==========================================
# Dictionary Extraction for Comprehensive Trait List
# ==========================================
...
unique_traits_dictionary <- try_raw %>%
  ...
  distinct() %>%
  # 5. Exclude numerical traits
  # suppressWarnings(as.numeric()) tries to convert the string to a number. 
  # If it is pure text (like "wind"), it becomes NA.
  # By keeping only the NAs, we automatically filter OUT all the pure numeric measurements!
  filter(is.na(suppressWarnings(as.numeric(OrigValueStr)))) %>%
  # 6. Sort for readability
  arrange(TraitName, OrigValueStr) %>%
  # 7. Add an empty column intended for LLM manual mapping
  mutate(Clean_Category = "")

# Export the dictionary template
rtry_export(unique_traits_dictionary, file.path(basepath, "output/taxonlist", "traits_dictionary_template.csv"))
